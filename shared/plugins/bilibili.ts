...
import { mediaBaseURL, requestHeaders, searchBaseURL } from "~~/constants";
...
async function getTrackUrl(id: string) {
  const bvid = id;
  const target_url = isServer ? mediaBaseURL.bbOfficial : mediaBaseURL.bbOfficial.replace("https://api.bilibili.com", "/api/bb");
  const target_url2 = isServer ? mediaBaseURL.bbOfficialm2 : mediaBaseURL.bbOfficialm2.replace("https://api.bilibili.com", "/api/bb");
  const resp1 = await $fetch<CidRes>(target_url, {
    method: "GET",
    headers: {
      Origin: requestHeaders.bilibili,
    },
    params: {
      bvid,
    },
...
async function search(keyword: string) {
  const target_url = isServer ? searchBaseURL.bbSearch : searchBaseURL.bbSearch.replace("https://api.bilibili.com", "/api/bb");

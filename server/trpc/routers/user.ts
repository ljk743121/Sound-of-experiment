...
export const userRouter = router({
  ...
  tokenValidity: protectedProcedure.query(() => {}), // protectedProcedure will check if user is logged in

  adminValidity: adminProcedure.query(() => {}),

  list: adminProcedure.use(requirePermission(["manageUser"])).query(async () => {
  ...
  listSongs: adminProcedure.use(requirePermission(["manageUser"])).query(async () => {
    return await db.query.users.findMany({
      orderBy: desc(users.createdAt),
      columns: {
        id: true,
  ...
  listUser: adminProcedure.use(requirePermission(["manageUser"])).query(async () => {
    return await db.query.users.findMany({
      orderBy: desc(users.createdAt),
      columns: {
        id: true,
  ...
  editPermission: adminProcedure
    ...
    .use(requirePermission(["manageUser", "editPermissions"]))
    .mutation(async ({ input }) => {
      await db.update(users).set({ permissions: input.permissions }).where(eq(users.id, input.id));
    }),
  editMaxSongs: adminProcedure
    ...
    .mutation(async ({ input }) => {
      await db.update(users).set({ maxSubmitSongs: input.maxSongs }).where(eq(users.id, input.id));
    }),

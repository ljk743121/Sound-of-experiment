<template>
  <div class="container max-w-full divide-y px-0 select-none md:border-x">
    <div class="flex items-center gap-2 pl-5 font-bold">
      ...
      <div class="ml-auto flex">
        <div
          class="flex flex-1 flex-col justify-center gap-1 border-l px-6 py-4 text-left sm:px-8 sm:py-6"
        >
          <span class="text-xs text-muted-foreground">累计投稿</span>
          <span class="text-lg leading-none font-bold sm:text-3xl">{{ countData?.songCount }}</span>
        </div>
        <div
          class="flex flex-1 flex-col justify-center gap-1 border-l px-6 py-4 text-left sm:px-8 sm:py-6"
        >
          <span class="text-xs text-muted-foreground">用户数目</span>
          <span class="text-lg leading-none font-bold sm:text-3xl">{{ countData?.userCount }}</span>
        </div>
      </div>
    </div>
    <div>
          ...
          <div v-for="(week, i) of weekData" :key="week.date" class="flex flex-col gap-2">
            <div class="relative flex h-[500px] w-8 flex-col justify-end md:h-[600px] md:w-16">
              <div
                ...
                :class="[
                  i % 2 === 0 ? 'bg-green-800 dark:bg-green-700' : 'bg-green-700 dark:bg-green-600',
                ]"
              >
            ...
            >
              ...
              <div class="flex w-full">
                <div
                  v-if="singer.count"
                  :style="{
                    width: `${((singer.count ?? 0) / singerMax) * 100}%`,
                  }"
        ...
        <ScrollArea class="h-[500px]" type="always">
          <div class="flex flex-col gap-2">
            <div v-for="(song, i) of likeData" :key="song.name" class="items-center gap-3 md:flex">
              <div
                class="flex items-center truncate text-xs text-muted-foreground md:w-40 md:text-right"
              >
                <Avatar class="mr-2 size-12 rounded">
                  <NuxtImg
              ...
              <div class="flex w-full">
                <div
                  v-if="song.count"
                  :style="{ width: `${((song.count ?? 0) / likeMax) * 100}%` }"
                  class="flex h-6 items-center rounded text-xs text-white"
                  :class="[
